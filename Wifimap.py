...
velocidad_mpm = 75 if tipo_red == "walk" else 250 if tipo_red == "drive" else 833
...
def obtener_wifi(distrito):
    ...
    return df.drop_duplicates(subset=["latitud", "longitud"]).reset_index(drop=True)
...
if respuesta and respuesta.get("last_clicked"):
    ...
    if grafo is None:
        st.warning("No se pudo cargar la red vial.")
        st.stop()

    try:
        nodo_origen = ox.distance.nearest_nodes(grafo, lon_user, lat_user)
    except:
        st.error("No se encontró nodo más cercano a tu ubicación.")
        st.stop()
    ...
    if mejor_ruta:
        ...
        st.warning("No se encontró una ruta conectada al punto WiFi.")
